...
/*   Updated: 2024/04/08 13:23:03 by romina           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	hendle_error(t_buffer *buffer)
{
							...
							t_list **history, t_list **env_var)
{
	...
	ft_bzero(buff, 10);
	buffer = init_buffer(*clipboard, search_var(*env_var, "?"));
	save_curr_line = NULL;
	old_termios = raw_mode();
	print_prompt(search_var(*env_var, "?"));
	ret = wait_line(buff, &buffer, &save_curr_line, history);
	if (ret == ETX)
		handle_ctrl_c(&buffer, env_var);
